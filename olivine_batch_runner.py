...
def run_windowed_slaves(cmd: str, session_name: str, prefix: str, persist: bool):
    ...
    for n in range(1, slave_count + 1):
        ...
        if persist:
            execute(f'tmux new-session -A -s {session_name} -n {prefix}-{n} "{new_cmd}; /bin/bash"')
        else:
            execute(f'tmux new-session -A -s {session_name} -n {prefix}-{n} "{new_cmd}"')
...
def populate_with_slave(n: str, fuzz_target_path: str, jit_compiler_code: str, until_n_inputs: int, seed: int):
    ...
    execute(f'''bash -c "{{ time ./fuzz/afl/afl-fuzz -s {seed} -e {until_n_inputs} -m none -o output-{n} -i ./corpus/output-{n} '{fuzz_target_path}' {lib_string} @@ ; }} 2> >(tee ~/die/output-{n}/time-populate.txt >&2)"''')
...
def fuzz(jit_compiler_code: str, until_n_inputs: int, seed: int):
    ...
    cmd: list[str] = [
        'cd ~/die',
        f'''bash -c "{{{{ time ./fuzz/afl/afl-fuzz -s {seed} -e {until_n_inputs} -j {jit_compiler_code} -m none -o output '{fuzz_target_path}' {lib_string} @@ ; }}}} 2> >(tee ~/die/output/time-fuzz.txt >&2)"''',
        f'bash -c "{{{{ time python3 ~/die/olivine_slave_analysis.py optset {{SLAVENUMBER}} {jit_compiler_code} ; }}}} 2> >(tee ~/die/output/time-analyze-optset.txt >&2)"',
