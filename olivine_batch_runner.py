...
def populate(fuzz_target_path: str, jit_compiler_code: str, seed: int, until_n_inputs: int):
    ...
    cmd = f'cd ~/die ; {{ time ./fuzz/afl/afl-fuzz -s {seed} -e {until_n_inputs} -m none -o output -i ./corpus/output "{fuzz_target_path}" {lib_string} @@ ; }} 2> >(tee ~/die/output/time-populate.txt >&2)'
    run_slaves(cmd, 'populate')


def fuzz(fuzz_target_path: str, jit_compiler_code: str, seed: int, until_n_inputs: int):
    lib_string = get_lib_string(jit_compiler_code)
    cmd = f'cd ~/die ; {{ time ./fuzz/afl/afl-fuzz -s {seed} -e {until_n_inputs} -m none -o output "{fuzz_target_path}" {lib_string} @@ ; }} 2> >(tee ~/die/output/time-fuzz.txt >&2)'
